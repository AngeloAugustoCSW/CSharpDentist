...
namespace Screens
{
    public class FormPatientListScrn : Form
    {   
        ...
        ListView listView;
        public FormPatientListScrn()
        {
            this.lblUser = new Fields.LabelFieldTam("Pacientes", 230, 15, 150, 30);
			...
			foreach (Paciente i in PacienteController.VisualizarPaciente())
            {
                ListViewItem item = new ListViewItem(i.Id + "");
                item.SubItems.Add(i.Nome);  
                item.SubItems.Add(i.Cpf);
                item.SubItems.Add(i.Fone);
                item.SubItems.Add(i.Email);
                item.SubItems.Add(i.DataNascimento + "");
			...
			listView.Columns.Add("Dt. Nascimento", -2, HorizontalAlignment.Left);
        ...
        {
            FormUpPatientScrn UpPatientScrn = new FormUpPatientScrn();
            UpPatientScrn.ShowDialog();
        } 
        public void btnInsertClick(object sender, EventArgs e)
        {
            FormInPatientScrn InPatientScrn = new FormInPatientScrn();
            InPatientScrn.ShowDialog();
        }
        public void btnDeleteClick(object sender, EventArgs e)
        {
            string message = "Voce deseja remover o registro do paciente?";
            ...
            {
                MessageBox.Show("Paciente removido com sucesso!", "Exclusão");
            } 
        }
    }
}